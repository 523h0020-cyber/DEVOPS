...
# Verify and reload Nginx (HTTP-only config)
sudo nginx -t
...
# 4. Update Nginx configuration with SSL (now that certificates exist)
echo "🔒 [4/4] Đang cập nhật Nginx với SSL certificates..."
sudo tee $NGINX_CONF > /dev/null <<EOF
server {
    listen 80;
    server_name $DOMAIN www.$DOMAIN;
...
server {
    ...
    # SSL Certificate paths (now they exist!)
...
# Verify and reload Nginx with SSL config
sudo nginx -t
sudo systemctl restart nginx
